...
rem Requires em++ (Emscripten) on PATH (emsdk: install latest, activate latest, emsdk_env.bat).
...
rem   em++ -O2 -std=c++20 -fwasm-exceptions -Icore/include -Ithird_party \
...
where em++ >nul 2>nul
if errorlevel 1 (
    echo error: em++ not found on PATH. Install and activate the Emscripten SDK:>&2
...
rem wrapper catches; without it, Emscripten's default (exceptions disabled) turns
rem every throw into an abort before the catch runs.
call em++ -O2 -std=c++20 -fwasm-exceptions ^
